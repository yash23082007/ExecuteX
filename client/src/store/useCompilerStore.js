...
const useCompilerStore = create(
  persist(
    (set, get) => ({
      ...
      aiReview: { loading: false, open: false, suggestions: [], error: null },
      ...
      runCode: async () => {
        ...
        try {
          ...
          if (!res.ok) {
            ...
            // Handle degraded state from the server (503)
            if (res.status === 503 && errData.degraded) {
              set({
                output: `⚠️ ${errData.error || "Execution service is temporarily unavailable. Please try again later."}`,
                hasError: true,
                isRunning: false,
              });
              return;
      ...
      checkAIStatus: async () => {
        try {
          ...
          const data = await res.json();
          set({ aiStatus: { available: !!data.available, checked: true } });
        } catch {
          set({ aiStatus: { available: false, checked: true } });
        }
      },

      toggleAISuggestions: () => {
        const { aiStatus, aiSuggestionsEnabled } = get();
        if (!aiStatus.available) return;
        set({ aiSuggestionsEnabled: !aiSuggestionsEnabled });
      },

      runAIReview: async () => {
        const { selectedLanguage, code, aiStatus } = get();
        if (!aiStatus.available || !code.trim()) return;

        set({ aiReview: { loading: true, open: true, suggestions: [], error: null } });
        ...
        try {
          ...
          const data = await res.json();
          if (data.success) {
            set({ aiReview: { loading: false, open: true, suggestions: data.suggestions || [], error: null } });
          } else {
            set({ aiReview: { loading: false, open: true, suggestions: [], error: data.error || "Review failed." } });
          }
        } catch {
          set({ aiReview: { loading: false, open: true, suggestions: [], error: "Failed to connect to AI service." } });
        }
      },

      clearAIReview: () => set({ aiReview: { loading: false, open: false, suggestions: [], error: null } }),
    }),
    {
      name: "executex-editor-storage",
      partialize: (state) => ({
        code: state.code ? state.code.slice(0, 10000) : "",
