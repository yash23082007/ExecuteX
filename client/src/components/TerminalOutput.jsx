...
function formatOutput(text, isError) {
  if (!text) return null;
  if (!isError) return text;
  return text.split('\n').map((line, i) => {
    if (line.toLowerCase().startsWith('error')) return <span key={i} className="out-error" style={{color: '#f87171'}}>{line}{'\n'}</span>;
    if (line.toLowerCase().startsWith('warning')) return <span key={i} className="out-warn" style={{color: '#fbbf24'}}>{line}{'\n'}</span>;
    if (/^\s*\|/.test(line)) return <span key={i} className="out-gutter" style={{opacity: 0.7}}>{line}{'\n'}</span>;
...
export default function TerminalOutput() {
  ...
  const ext = FILE_EXT[selectedLanguage] || "";
      ...
      <div className={`termpanel__body ${hasError && activeTab === 'output' ? "termpanel__body--err" : ""}`} ref={bodyRef}>
        {activeTab === 'input' ? (
          ...
          <div className="term__result">
            ...
            <pre className="term__out font-mono">{formatOutput(output, hasError)}</pre> 
